...
//print visits , takes an array of pointer and number of pages which is 4)
{
    ...
    for (int i = 0; i < numOfPages; i++) {
        //get pageObject's visits by -> as its a pointer, calculate out percentage specifically to 2 decimal.
        percentage = (static_cast<double>(pages[i]->getNumVisits()) / 1000000) * 100.0; 
        // now cout nice and clear
        cout << "Page " << i + 1 << " contributes " << fixed << setprecision(2) << percentage << "% of visits" << std::endl;
    }
}
...
for(int i = 0;i <1000000; i++)// do the following 1mil times
{
    int rand1_100 = rand()%100+1;  // i roll 1~100
        // if i roll <= 15 or the page has no link yet, I assign a random page form 4.
        if (rand1_100 <=15 || currentPage->getNumLinks()==0){
             currentPage = pages[rand() % 3];
        }// otherwise
        else{
            currentPage = currentPage->getRandomLink();// get random link with member function and link to current.
        }
     currentPage->visit();
};
